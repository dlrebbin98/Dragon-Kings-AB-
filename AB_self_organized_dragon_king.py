...
class Degree_Cascade:
    ...
    def recursive_cascade(self, node):
        ...
        # Locate neighbors
        all_neighbors = self.network.get_neighbors(node, as_list=True)
        ...
        neighbor_statuses = np.array(list(self.network.get_statuses(all_neighbors)))

        target_nodes = np.where(neighbor_statuses - prev_status < 1)[0]

        for target in target_nodes:
            self.recursive_cascade(target)
